...
short_description: Manage MikroTik RouterOS configuration sections
description:
  - RouterOS configurations use a simple block scoped file syntax
    for segmenting configuration into sections.  This module provides
    an implementation for working with IOS configuration sections in
    a deterministic way.
extends_documentation_fragment: routeros
notes:
  - Tested against RouterOS 6.0.0
...
# that is returned from the `routeros_facts` module
...
backup_path:
  ...
  type: str
  sample: /playbooks/ansible/backup/routeros_config.2016-07-16@22:28:34
filename:
  description: The name of the backup file
  returned: when backup is yes and filename is not specified in backup options
  type: str
  sample: routeros_config.2016-07-16@22:28:34
shortname:
  description: The full path to the backup file excluding the timestamp
  returned: when backup is yes and filename is not specified in backup options
  type: str
  sample: /playbooks/ansible/backup/routeros_config
date:
...
from ansible.module_utils.network.routeros.routeros import run_commands, get_config
from ansible.module_utils.network.routeros.routeros import routeros_argument_spec
from ansible.module_utils.network.routeros.routeros import get_defaults_flag, get_connection
from ansible.module_utils.network.routeros.routeros import CustomNetworkConfig
from ansible.module_utils.basic import AnsibleModule
from ansible.module_utils.network.common.config import dumps

display = Display()
display.vvvv("starting run")


def check_args(module, warnings):
    # routeros_check_args(module, warnings)
...
def main():
    ...
    if module.params['backup'] or (module._diff and module.params['diff_against'] == 'running'):
        ...
        config = CustomNetworkConfig(indent=1, contents=contents)
    ...
    if any((module.params['lines'], module.params['src'])):
        ...
        display.debug("candidate=candidate")
        display.debug(candidate)
        display.debug("running=running")
        display.debug(running)
        display.debug("diff_match=match")
        display.debug(match)
        display.debug("diff_ignore_lines=diff_ignore_lines")
        display.debug(diff_ignore_lines)
        display.debug("path=path")
        display.debug(path)
        display.debug("diff_replace=replace")
        display.debug(replace)
    ...
    if module.params['save_when'] == 'always':
        ...
        running_config = CustomNetworkConfig(
            indent=1, contents=output[0], ignore_lines=diff_ignore_lines)
        startup_config = CustomNetworkConfig(
    ...
    if module._diff:
        ...
        if contents is not None:
            base_config = CustomNetworkConfig(
                indent=1, contents=contents, ignore_lines=diff_ignore_lines)
